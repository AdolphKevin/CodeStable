---
name: cs-note
description: 把"短到不值得起一份文件、但 AI 每次启动 CodeStable 技能都必须知道"的自动提醒项追加到 `.codestable/attention.md` 的固定分节里——比如编译特殊 flag、运行前要先起的服务、路径陷阱、命令别名、环境变量约定。触发：用户说"记一笔"、"加到 attention.md"、"项目要 X 才能编译"、"以后每次都得 Y"，或知识沉淀判断识别出自动提醒项。
---
...
cs-learn / cs-trick / cs-decide 产出独立 markdown 文件，**通过检索**被读到；`.codestable/attention.md` 是 CodeStable 技能启动时的**强制必读**上下文。这两类信息归宿不同——本技能专管后者：把"短、稳、每次都要知道"的自动提醒项追加到 attention 文件里。

本技能是归档执行器，不替代沉淀类技能。收尾阶段的"知识沉淀判断"识别出自动提醒项时，由 AI 路由到这里落盘；用户只确认这条信息是否每次会话都该自动提醒。
...
1. **用户在对话中说出明显属于碎片知识的事实**——"哦对这个项目要先 X 才能 Y"、"我们这个用 Z 不用 W"——推："这条属于自动提醒项候选，以后 AI 每次都能看到，要保留吗？"
2. **AI 自己刚踩了一个一句话能讲清的项目特殊设置**（编译失败 / 命令不对 / 路径找错）——修复后推："这条属于自动提醒项候选吗？是的话保留到 attention.md，下次会话直接知道。"
