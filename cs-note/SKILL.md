---
name: cs-note
description: 把"短到不值得起一份文件、但 AI 每次启动 CodeStable 技能都必须知道"的自动提醒项追加到 `.codestable/attention.md` 的固定分节里——比如编译特殊 flag、运行前要先起的服务、路径陷阱、命令别名、环境变量约定。触发：用户说"记一笔"、"加到 attention.md"、"项目要 X 才能编译"、"以后每次都得 Y"，或知识沉淀盘点识别出自动提醒项。
---
...
本技能是归档执行器，不替代沉淀类技能。收尾阶段的"知识沉淀盘点"识别出自动提醒项时，由 AI 路由到这里落盘；用户只确认这条信息是否每次会话都该自动提醒。
