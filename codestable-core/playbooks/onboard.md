...
6. Generate `.codestable/specs/INDEX.md` as scoped engineering standards discovered from manifests, CI, tests, existing docs, and owner-confirmed rules.
7. Generate `.codestable/tasks/INDEX.md` as the task-memory entry point for future work.
8. Generate `.codestable/attention.md` with only operational facts observed from manifests/config/tests; do not invent project owner preferences.
9. Copy runtime reference/tools from the package.
...
If Python is unavailable, manually produce the same two artifacts with equivalent content. Always exclude `.git/`, `node_modules/`, build outputs, virtualenvs, cache directories, and `.codestable/` itself from code inventory. After inventory, create an initial reliability note that states which checks are detected and which are unknown.
...
4. Create code-informed `INDEX.md`, `ARCHITECTURE.md`, `VISION.md`, `specs/INDEX.md`, `tasks/INDEX.md`, `compound/INDEX.md`, and `attention.md` using `codestable-core/onboard/reference.md` templates.
5. Report observed facts, unknowns, owner-confirmation candidates, first scoped specs, and any existing active work.
...
2. Re-read `INDEX.md`, `ARCHITECTURE.md`, `VISION.md`, `specs/INDEX.md`, `tasks/INDEX.md`, `compound/INDEX.md`, `attention.md`.
...
- User-maintained architecture/requirements/specs/compound/roadmap/task journals are never deleted during onboard.
...
Scoped Specs: <created/updated/none>
Task Memory Root: <created/updated/none>
...
- Do not delete or move user-maintained docs, specs, task journals, or roadmap artifacts during onboard.
