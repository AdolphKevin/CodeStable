...
同时读 `.codestable/INDEX.md`、`.codestable/attention.md` 和本次改动触碰到的相关索引（`architecture/ARCHITECTURE.md`、`requirements/VISION.md`、`compound/INDEX.md`）。索引文档只是入口，真正检查要读到被引用的具体文档。
...
- 项目总索引：总是读 `.codestable/INDEX.md`
- 项目硬约束：总是读 `.codestable/attention.md`
- 结构边界：先读 `architecture/ARCHITECTURE.md`，再打开相关具体 architecture 文档
- 能力边界：先读 `requirements/VISION.md`，再打开相关具体 requirement 文档
- 已拍板知识：先读 `compound/INDEX.md`，再按 `doc_type` 打开 decision / learning / trick / explore

检索规则：

- 先读索引：`INDEX.md`、`architecture/ARCHITECTURE.md`、`requirements/VISION.md`、`compound/INDEX.md`
...
the knowledge-sync workflow / the knowledge-sync workflow / the knowledge-sync workflow / the knowledge-sync workflow 共享下面这组规则。子技能正文只写特有反模式，通用看这里：
...
4. **attention.md 检查**——写完后若沉淀暴露出"每次启动都该知道"的一两行硬约束，内部路由到自动提醒归档；对用户说"已加入启动必读提醒"或"这条是否每次启动都该提醒需要你确认"。不要提示用户执行 the knowledge-sync workflow，也不要直接改外部 AI 入口
...
the feature workflow 和 the issue workflow 共用。AI 默认会往"大函数 / 大文件 / god class / 处处特殊分支"漂，这一节把漂移截在发生那一刻。
