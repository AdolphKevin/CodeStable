...
| 新功能 / "加个 X" / "实现 XX" | 默认快速实现；跨模块 / 新能力边界 / 高风险时再进入标准功能流程 |
| BUG / 异常 / 报错 / "这里不对" / "文档错了" | 默认快速定位修复；根因不明 / 多模块影响时再补完整问题流程 |
| 代码优化 / 重构 / 重写（行为不变） | 默认小重构；跨模块 / 公开接口 / 无测试时再进入标准重构流程 |
| 摸代码 / "X 是怎么实现的" / 提问调研 | 做一次代码探索，给结论和证据 |
| 业务流程梳理 / "画出 X 流程图" / "说明 X 怎么处理" | 梳理业务处理链路，输出流程图和解释 |
| 代码写完 / 做质量检查 / 准备验收 / 准备 merge | 对照本次变更、方案产物和项目规则做质量检查；feature 进入验收，issue 进入修复验证 |
| 提交代码 / 生成 commit message / staged diff 已准备好 | 生成并执行一次 scoped commit |
| 收尾 / 今天先到这 / wrap up | 先确认本次代码已检查并提交；未提交则回 scoped-commit，已提交再只做知识 / 文档状态确认 |
