...
重构默认走小重构：范围小、有测试能自证，就直接改。标准流程只给范围大、风险高、需要用户取舍的重构用。

AI 自己重构有两个稳定失败模式：一是不知道模块真实需求和约束，改出来的东西功能不等价；二是一次吞掉的范围超过上下文承载，改到后面忘了前面的约束。标准流程在"想优化"和"动手改"之间塞扫描清单 + 方法库，让 AI 只接自己能稳定做对的活。
...
单函数 / 单组件 / 1-3 个文件 / 有测试可自证 / 不需要目视——默认触发 `cs-refactor-ff`：直接识别、一次对齐、原地改、跑测试自证，不产 scan / design / checklist。

触发："小重构"、"快速重构"、"简单优化下 XX 函数"、"直接改"、"别那么多步骤"。

**别走** ff：预计动点 > 3 处 / 需要目视验证 / 改公开接口（要 Parallel Change）/ 没有测试覆盖 / 跨模块。遇到劝用户走标准流程。ff 开干后发现变复杂切回完整流程从 scan 开始。
